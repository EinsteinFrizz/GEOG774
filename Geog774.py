...
"""
#STUFF TO DO
#need to check that drop table is indeed clearing the table as otherwise every time the data is run it will just be added to the end of the table
#might need to redo index because it is independent of the tables (drop index?)
# %% IMPORTING PACKAGES
...
params = {'latitude':'-36.842462' , 'longitude':'174.755304' , 'limit':'50' , 'radius':'500'}
...
curr.execute(sql) #send sql command
...
sql = "DROP INDEX IF EXISTS geom_index; CREATE INDEX geom_index ON public.yelp_data USING gist (geom) TABLESPACE pg_default; " #index allows faster data access
...
for b in payload_review[1:] : #[1:] starts after header row
...
model = models. ldamodel.LdaModel(corpus_tfidf, id2word=dictionary, alpha=0.001, num_topics=20, update_every=0, passes=50)
...
#    n = len(f['reviews'])
#    review1,review2,review3 = 'null','null','null' #initialises variables
#    if n > 0: review1 = f['reviews'][0]['text'] #adds 'text' value to the list below if it exists
#    if n > 1: review2 = f['reviews'][1]['text'] #adds 'text' value to the list below if it exists
#    if n > 2: review3 = f['reviews'][2]['text'] #adds 'text' value to the list below if it exists
#    
...
#logging.info("starting LDA model")
#
#model = models. ldamodel.LdaModel(corpus_tfidf, id2word=dictionary, alpha=0.001, num_topics=10, update_every=0, passes=5)
#
#pp(model.show_topics(formatted=True))
#
## %% OUTPUT TO CSV
